...
router.post("/", async(req, res) => {
    ...
    try {
        const resolvedOrderItems = await orderItemsIds;
        const totalPrices = await Promise.all(
            resolvedOrderItems.map(async(oi) => {
                const orderItem = await OrderItem.findById(oi).populate(
                    "product",
                    "price"
                );
    ...
    try {
        ...
        if (order) {
            await order.orderItems.map(
                async(o) => await OrderItem.findByIdAndDelete(orderItem)
            );
            return res
                .status(200)
                .json({
            ...
            return res
                .status(404)
                .json({ success: false, message: "The order hasn't been founded!" });
        }
    } catch (error) {
        console.log(error);
